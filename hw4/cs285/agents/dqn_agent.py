...
class DQNAgent(object):
    ...
    def add_to_replay_buffer(self, paths, **kwargs):
        pass
    ...
    def train(self, ob_no, ac_na, re_n, next_ob_no, terminal_n):
        ...
        self.replay_buffer.can_sample(self.batch_size))):
            ...
            with tf.GradientTape() as tape:
                ...
                current_state_actions_mask = tf.one_hot(act_batch, depth=self.num_actions)
                q_target = current_state_q_a * (1.0 - current_state_actions_mask) + tf.expand_dims(q_target, axis=1) * current_state_actions_mask
                q_target = tf.stop_gradient(q_target)
                # pred_q = tf.reduce_sum(current_state_q_a * tf.one_hot(act_batch, depth=self.num_actions), axis=1)
                loss_value = self.q_t_loss(q_target, current_state_q_a)
